...
t.test(mining, water, conf.level = .9, mu=0.035)

shapiro.test(water)
shapiro.test(mining)

plot(water, dnorm(water, mean(water), sd(water)))
title(main="Graph of water share with respect to the total volume")

plot(mining, dnorm(mining, mean(mining), sd(mining)))
title(main="Graph of mining share with respect to the total volume of production")
